...
embed_name = 'lookerembed_cloudtop'
embed_secret = data[f'{embed_name}']['embed_secret']

ini_path = './app/dash_params.json'
if os.path.exists(ini_path):
  with open(ini_path, 'r') as f:
    dash_params = json.loads(f.read())

  # section = 'embed'
  # p_external_user_id = dash_params[f'{section}'][f'external_user_id']
  # p_first_name = dash_params[f'{section}'][f'first_name']
  # p_last_name = dash_params[f'{section}'][f'last_name']
  # p_permissions = dash_params[f'{section}'][f'permissions']
  # p_models = dash_params[f'{section}'][f'models']
  # p_external_group_id = dash_params[f'{section}'][f'external_group_id']
  # p_embed_dashboard_id = dash_params[f'{section}'][f'embed_dashboard_id']
  # p_embed_target_url = dash_params[f'{section}'][f'embed_target_url']

  # print('external_user_id:' + str(p_external_user_id))
  # print('first_name:' + p_first_name)
  # print('last_name:' + p_last_name)
  # print('permissions:' + p_permissions[0])
  # print('models:' + p_models[0])
  # print('external_group_id:' + p_external_group_id)
  # print('embed_dashboard_id:' + str(p_embed_dashboard_id))
  # print('embed_target_url:' + p_embed_target_url)
...
def generate():
  ...
  user = User(
    ...
    dash_params[f'{section}'][f'external_user_id'],
    first_name = dash_params[f'{section}'][f'first_name'],
    last_name = dash_params[f'{section}'][f'last_name'],
    permissions = dash_params[f'{section}'][f'permissions'],
    models = dash_params[f'{section}'][f'models'],
    external_group_id = dash_params[f'{section}'][f'external_group_id'],
  )

  embed_dashboard_id = dash_params[f'{section}'][f'embed_dashboard_id']
  embed_target_url = dash_params[f'{section}'][f'embed_target_url']
  
  # looker = Looker('masa.dev.looker.com', embed_secret)
  looker = Looker(embed_target_url, embed_secret)

  fifteen_minutes = 15 * 60

  url = URL(looker, user, fifteen_minutes, "/embed/dashboards-next/" + str(embed_dashboard_id) +"?embed_domain=http://127.0.0.1:5000", force_logout_login=True)

  return("https://" + url.to_string())


def generate_formatted():
  section = 'embed_formatted'
  user = User(
    # '999',
    # first_name='Embed',
    # last_name='Looker',
